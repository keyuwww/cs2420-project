#!/usr/bin/env python3
"""
Fix the incomplete all_metrics.json file from training
"""
...
# The file likely has train_history and val_history but failed on test
lines = content.split('\n')
train_history = []
val_history = []

# Parse manually or reconstruct from what we know
# From the log, we know the test results:
test_results = {
    "loss": 0.0563,
    "accuracy": 0.9833,
    "auroc": 0.9948,
...
# Try to parse the JSON up to where it failed
try:
    ...
    # If parsing fails, reconstruct from log output
    ...
    # Val history would be in the partial file, but we'll use a placeholder
    val_history = []

# Reconstruct the complete metrics
complete_metrics = {
