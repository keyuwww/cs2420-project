...
if "ANTHROPIC_API_KEY" not in os.environ:
    ...
    print("Please set: export ANTHROPIC_API_KEY=your_api_key_here", file=sys.stderr)
    sys.exit(1)
        ...
        try:
            ...
            if attempt < max_retries - 1:
                ...
                if hasattr(e, 'response') and e.response is not None:
                    ...
                    if retry_after:
                        retry_delay = max(retry_delay, float(retry_after) + 2)
                elif "retry" in str(e).lower():
                    delay_match = re.search(r'retry[^\d]*(\d+)', str(e), re.I)
                    if delay_match:
                        retry_delay = max(retry_delay, float(delay_match.group(1)) + 2)
